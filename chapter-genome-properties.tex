...
InterPro identifier (e.g., IPRXXXX) that is mapped to multiple identifiers for 
the same protein or domain within member databases (e.g., PFXXXX or TFXXXX) 
\cite{hunter2008interpro,finn2016interpro}.

An import feature of protein databases is the ability to take the sequence of a 
novel protein (i.e., a protein that is not currently in the database) and 
...
consortium compare novel proteins to computational models (i.e., a profile) that 
represents the sequence diversity (i.e., not all domains from different proteins 
...
it is that the match is a real (i.e., the organism's protein contains the 
...
model-based sequence search techniques (e.g., HMMER \cite{eddy2011accelerated}) 
...
and the organism's genome size. InterProScan takes a FASTA file 
...
such as cell motility (e.g., the presence flagella) and even microbial viral 
...
an organism's genome such as the presence InterPro domains (e.g., \gls{pfam} 
...
(e.g., those produced by Prodigal), and these annotations are then combined with 
...
\item A \textbf{FASTA} \cite{pearson19905} file containing representative 
...
\gls{pfam} and Rfam databases \cite{bateman2004pfam, griffiths2003rfam}. Like 
...
or RPS-BLAST \cite{mcginnis2004blast}. When compared to \gls{blast}-based 
\cite{altschul1990basic} search methods, which are commonly used by other 
pathway annotation systems, these model-based tools are better at detecting 
enzymes whose sequences are phylogenetically divergent from those previously 
known \cite{eddy2011accelerated}. This improved detection capability provides 
Micromeda with an advantage when it is applied to the genomes of previously 
unstudied organisms.
