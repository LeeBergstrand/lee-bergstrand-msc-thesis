...
\begin{figure}[!ht]
  \centering
	\includegraphics[width=0.9\textwidth]{media/Coloured-Endpoint.pdf}
	 \caption{The example URL above is used to download a FASTA file containing the top proteins (i.e., those with lowest E-value domains) that support GenProp0526 step one for dataset FXDABADS. The URL path variables are in blue and the HTTP GET parameters are in green. Note that the URL above is an example and does not point towards Micromeda-Server or Micromeda-Client. Links to a demonstration of the client interface can be found in Section \ref{client-demo}.}
