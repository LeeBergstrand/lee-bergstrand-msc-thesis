...
using a series of \gls{url} addresses \cite{berners1994rfc} (i.e., web 
...
\gls{url} addresses (i.e., ``endpoints"). Information about the Genome 
...
file containing the top proteins (i.e., those with the lowest \gls{eval} 
...
of it and a list of equivalent records from other databases (e.g., \gls{kegg} 
...
records from other databases (e.g., \gls{kegg}, MetaCyc) \\ \hline
...
the top proteins (i.e., those with the lowest \gls{eval} domain annotations) 
...
list of assignments of support (i.e., YES, NO, PARTIAL), one for each sample in 
...
required, and users must tell Micromeda-Server to use their dataset (i.e., a 
