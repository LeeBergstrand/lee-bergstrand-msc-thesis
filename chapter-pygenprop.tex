...
Out: A list of InterPro identifiers (\textit{e}.\textit{g}., IPR011776)

step.consortium_identifiers 
Out: A list of consortium signature identifiers (\textit{e}.\textit{g}., TIGR02031)
...
InterPro consortium database signatures (\textit{e}.\textit{g}., \gls{pfam}s, \gls{tigr}s,  and 
...
AssignmentCache objects contain two Python dictionaries (\textit{i}.\textit{e}., key-value 
...
file parsed using Scikit-bio's \gls{fasta} file parser (Fig. 
...
single organism.} They are generated from a \gls{fasta} file of an organism's proteins 
...
used to generate \gls{fasta} files, using Scikit-bio, containing proteins that are 
...
process that continually runs on a computer system (\textit{i}.\textit{e}., they are daemons 
